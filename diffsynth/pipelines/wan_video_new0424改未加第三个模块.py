...
# 在 wan_video_new.py 顶部添加（大约第27行，在其他导入之后）
...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None):
        ...
        self.units = [
            ...
            WanVideoUnit_StructureEmbedder(),  # 🔥 添加在这里，在S2V之前
            WanVideoUnit_S2V(),
    ...
    def training_loss(self, **inputs):
        ...
        """训练损失计算 - 需要先通过 Units 预处理数据"""
        
        # 🔥 分离模型和数据
        model_names = self.in_iteration_models
        models_from_inputs = {name: inputs.pop(name) for name in model_names if name in inputs}
        
        # 🔥🔥🔥 分离prompt到inputs_posi（PromptEmbedder需要）
        prompt = inputs.pop("prompt", "")
        
        # 现在 inputs 只包含数据，不包含模型对象和prompt
        inputs_shared = inputs.copy()
        inputs_posi = {"prompt": prompt}  # 🔥 PromptEmbedder从这里获取
        inputs_nega = {"prompt": ""}       # 训练时不需要negative prompt
        ...
        WanVideoUnit_NoiseInitializer(),
        WanVideoUnit_PromptEmbedder(),
        WanVideoUnit_StructureEmbedder(),
        WanVideoUnit_InputVideoEmbedder(),
        ]
        
        # 执行预处理单元（只需要数据准备相关的）
        for unit in self.units:
            # print(f"[DEBUG] Executing unit: {unit.__class__.__name__}")
            if isinstance(unit, (WanVideoUnit_ShapeChecker, WanVideoUnit_NoiseInitializer, 
                            WanVideoUnit_PromptEmbedder, WanVideoUnit_StructureEmbedder,
                            WanVideoUnit_InputVideoEmbedder)):
        ...
        # 现在 inputs_shared 包含了 noise 和 input_latents
        max_timestep_boundary = int(inputs_shared.get("max_timestep_boundary", 1) * self.scheduler.num_train_timesteps)
        min_timestep_boundary = int(inputs_shared.get("min_timestep_boundary", 0) * self.scheduler.num_train_timesteps)
        timestep_id = torch.randint(min_timestep_boundary, max_timestep_boundary, (1,))
        timestep = self.scheduler.timesteps[timestep_id].to(dtype=self.torch_dtype, device=self.device)
        
        # ✅ 原版逻辑：直接使用 input_latents 和 noise
        inputs_shared["latents"] = self.scheduler.add_noise(inputs_shared["input_latents"], inputs_shared["noise"], timestep)
        training_target = self.scheduler.training_target(inputs_shared["input_latents"], inputs_shared["noise"], timestep)
        
        # 使用从inputs分离出的模型对象（如果有的话），否则从self获取
        if models_from_inputs:
            models = models_from_inputs
        else:
            models = {name: getattr(self, name) for name in self.in_iteration_models if getattr(self, name, None) is not None}
        
        # 🔥 合并context到inputs_shared（PromptEmbedder的输出）
...
# 🆕 添加结构编码器处理单元
class WanVideoUnit_StructureEmbedder(PipelineUnit):
    """结构条件编码器单元 - 处理线稿和分割图"""
    def __init__(self):
        super().__init__(input_params=("sketch_image", "seg_image", "height", "width"))
        self.required_inputs = ["sketch_image", "seg_image"]

    def process(self, pipe: WanVideoPipeline, sketch_image, seg_image, height, width):
        """处理线稿和分割图"""
        if pipe.structure_encoder is None:
            return {}
        
        from PIL import Image
        import os
        
        # 🔥🔥🔥 先检查是否为字符串路径，如果是则加载图像
        if isinstance(sketch_image, str):
            if not os.path.isabs(sketch_image):
                # 如果是相对路径，需要根据数据集base_path拼接
                # 但这里我们假设已经是绝对路径或者可以直接打开
                pass
            sketch_image = Image.open(sketch_image).convert('L')  # 转为灰度图
        
        if isinstance(seg_image, str):
            if not os.path.isabs(seg_image):
                pass
            seg_image = Image.open(seg_image).convert('RGB')  # 转为RGB
        
        # 🔥 处理 sketch（灰度图，1通道）
        if isinstance(sketch_image, Image.Image):
            sketch_image = sketch_image.resize((width, height), resample=Image.BILINEAR)
            sketch = torch.from_numpy(np.array(sketch_image)).float() / 255.0
            if sketch.dim() == 2:  # (H, W) -> (1, 1, H, W)
                sketch = sketch.unsqueeze(0).unsqueeze(0)
            elif sketch.dim() == 3:  # (H, W, 1) -> (1, 1, H, W)
                sketch = sketch.permute(2, 0, 1).unsqueeze(0)
            sketch = sketch.to(device=pipe.device, dtype=pipe.torch_dtype)
        else:
            raise ValueError(f"sketch_image must be PIL.Image or str, got {type(sketch_image)}")
        
        # 🔥 处理 seg（RGB，3通道）
        if isinstance(seg_image, Image.Image):
            seg_image = seg_image.resize((width, height), resample=Image.NEAREST)
            seg = torch.from_numpy(np.array(seg_image)).float() / 255.0
            if seg.dim() == 3:  # (H, W, 3) -> (1, 3, H, W)
                seg = seg.permute(2, 0, 1).unsqueeze(0)
            seg = seg.to(device=pipe.device, dtype=pipe.torch_dtype)
        else:
            raise ValueError(f"seg_image must be PIL.Image or str, got {type(seg_image)}")
        
        # 前向传播 (接收三个返回值)
        structure_context, dynamic_mask, time_decay_curve = pipe.structure_encoder(sketch, seg)
        
        return {"structure_context": structure_context, "dynamic_mask": dynamic_mask, "time_decay_curve": time_decay_curve}
    ...
    def process(self, pipe: WanVideoPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image):
        if input_video is None:
            ...
            if pipe.scheduler.training:
                return {"latents": noise, "input_latents": noise}
            else:
        ...
        if pipe.scheduler.training:
            # ✅ 原版逻辑：必须返回 latents, input_latents  
            return {"latents": noise, "input_latents": input_latents}
        else:
            # 推理模式：返回加噪后的 latents
            latents = self.scheduler.add_noise(input_latents, noise, timestep=pipe.scheduler.timesteps[0])
...
def model_fn_wan_video(
    ...
    structure_context: Optional[torch.Tensor] = None,  
    dynamic_mask: Optional[torch.Tensor] = None,   # 🔥 接收动态掩码
    time_decay_curve: Optional[torch.Tensor] = None,  # 🔥 接收深度学习产生的时间曲线
    **kwargs,
):
    if sliding_window_size is not None and sliding_window_stride is not None:
        model_kwargs = dict(
            ...
            motion_bucket_id=motion_bucket_id,
            structure_context=structure_context,  # 🆕 传递参数
            dynamic_mask=dynamic_mask,  # 🔥 传递动态掩码
            time_decay_curve=time_decay_curve, # 🔥 传递时间衰减曲线
        )
    ...
    # 🔥🔥🔥 必须解除注释：将结构编码器的特征与文本序列在长度维度拼接
    if structure_context is not None:
        # CrossAttention 可以无视 sequence length 变化
        # 将编码器输出当成额外的视觉 Prompt 拼在 Text Prompt 前面
        context = torch.cat([structure_context, context], dim=1)
    ...
    # 🔥🔥🔥 注入时序解耦逻辑：构建时空分布因子 (Spatio-Temporal Decay Field)
    st_decay = None
    if dynamic_mask is not None and time_decay_curve is not None:
        # 将 2D 骨架掩码放缩到与视频特征块匹配的 (h, w)
        dm = torch.nn.functional.interpolate(dynamic_mask, size=(h, w), mode='bilinear').to(x.dtype)
        # dm shape: [B, 1, h, w]
        
        # 🔥 切取模型推理出的专属于前 f 帧的衰减曲线，并整形
        t_gradient = time_decay_curve[:, :f].to(x.dtype) # [B, f]
        t_gradient = t_gradient.view(-1, f, 1, 1) # [B, f, 1, 1]
        
        # 时空相乘：静物区域 dm=0；水流云动区域则运用神经网络算出的平滑非线性曲线渐变
        st_decay_field = dm.unsqueeze(2) * t_gradient # [B, 1, f, h, w] -> reshape 为 [B, f, h, w, 1]
        st_decay = rearrange(st_decay_field, 'b c f h w -> b (f h w) c').contiguous() # [B, L, 1]
    ...
    if tea_cache_update:
        ...
        def create_custom_forward(module):
            # 🔥 修复解包：增加 st_decay 参数的传递
            def custom_forward(x_in, context_in, t_mod_in, freqs_in, sketch_context_in, st_decay_in):
                return module(x_in, context_in, t_mod_in, freqs_in, sketch_context=sketch_context_in, st_decay=st_decay_in)
            ...
            if use_gradient_checkpointing_offload:
                with torch.autograd.graph.save_on_cpu():
                    x = torch.utils.checkpoint.checkpoint(
                        ...
                        x, context, t_mod, freqs, structure_context, st_decay, # 🔥 添加这里
                        use_reentrant=False,
                    )
                ...
                x = block(x, context, t_mod, freqs, sketch_context=structure_context, st_decay=st_decay)
