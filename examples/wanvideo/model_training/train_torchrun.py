...
def launch_training_task_torchrun(
    ...
    # 🔥🔥🔥 修复处：将 import 提前到这里！确保 os 上下文可用
    ...
    if args.resume_from_checkpoint and os.path.exists(args.resume_from_checkpoint):
        if is_main_process:
            print(f"[INFO] 🔄 正在从断点恢复权重: {args.resume_from_checkpoint}")
        
        # 加载断点权重
        from safetensors.torch import load_file
        state_dict = load_file(args.resume_from_checkpoint)
        # 注意: 你的保存代码增加了 prefix，使用 strict=False 来兼容兼容性加载
        model.load_state_dict(state_dict, strict=False)
        del state_dict
        torch.cuda.empty_cache()
        
        # 尝试自动解析 step 步数 (假设文件名类似 step-2500.safetensors)
        basename = os.path.basename(args.resume_from_checkpoint)
        if "step-" in basename:
            try:
                global_step = int(basename.split("step-")[1].split(".")[0])
                
                # 计算需要断点恢复的纪元和批次进度
                start_epoch = global_step // steps_per_epoch
                resume_step_in_epoch = (global_step % steps_per_epoch) * args.gradient_accumulation_steps
                
                # 快进 LR Scheduler，使其恢复到中断时的学习率
                for _ in range(global_step):
                    scheduler.step()
                    
                if is_main_process:
                    print(f"[INFO] ▶️ 成功解析断点: 将跳过前 {start_epoch} 个 Epoch，从第 {start_epoch+1} 轮的第 {resume_step_in_epoch} 批次继续！")
            except Exception as e:
                if is_main_process:
                    print(f"[WARNING] 步数解析失败: {e}，将仅加载权重并从头开始计步。")
    ...
    if torch.distributed.get_rank() == 0:
        ...
        # 如果是断点续训，则追加(a)；如果是全新训练，则新建(w)
        file_mode = 'a' if global_step > 0 else 'w'
        with open(loss_log_path, mode=file_mode, newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            if file_mode == 'w':
                writer.writerow(["Time", "Epoch", "Global_Step", "Loss", "LR"])
            
    # 【修复 Bug】：统一使用 epoch 作为变量名，并应用断点 start_epoch
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for step, data in enumerate(pbar):
            ...
            try:
                ...
                # 🌟🌟🌟 创新点三：在 MoE 训练阶段加上协同正则化
                aux_loss = 0.0
                if args.train_moe:
                    # 遍历并聚合所有注入了 MoE Wrapper 的层产生的 rasr_loss
                    for m in model.modules():
                        if type(m).__name__ == 'MoECrossAttentionWrapper' and hasattr(m, 'rasr_loss'):
                            aux_loss += m.rasr_loss
                    

                    dsrf_aux = 0.0
                    if hasattr(model.pipe, "structure_encoder") and model.pipe.structure_encoder is not None:
                        if hasattr(model.pipe.structure_encoder, "dsrf_aux_loss") and model.pipe.structure_encoder.dsrf_aux_loss is not None:
                            dsrf_aux = model.pipe.structure_encoder.dsrf_aux_loss
                    # 乘以一个正则化系数 (通常设为 0.01~0.05 之间，不干扰主重建任务)

                    loss = loss + 0.02 * aux_loss + 0.01 * dsrf_aux
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.save_steps and global_step % args.save_steps == 0 and is_main_process:
                        ckpt_path = save_checkpoint(model, model_logger, global_step, args)
                        
                        # 🔥🔥🔥==================== 2. 新增: 后台可视化展示测试 ====================🔥🔥🔥
                        if args.run_validation and args.val_script and ckpt_path:
                            import subprocess
                            val_out_dir = os.path.join(args.output_path, "visualizations")
                            os.makedirs(val_out_dir, exist_ok=True)
                            
                            # 将推理输出日志重定向，以免打乱终端界面的 tqdm 进度条
                            val_log_file = os.path.join(val_out_dir, f"val_step_{global_step}.log")
                            
                            print(f"\n🎬 正在后台启动验证程序, 视频生成后将保存在 '{val_out_dir}'...")
                            
                            val_cmd = [
                                sys.executable, args.val_script,
                                "--ckpt_path", ckpt_path,           # 注意：这里的参数名需要与你的 inference_ti2v.py 匹配
                                "--output_dir", val_out_dir         # 注意：这里的参数名需要与你的 inference_ti2v.py 匹配
                            ]
                            
                            # 使用 Popen 拉起非阻塞后台子进程 
                            with open(val_log_file, "w") as f_log:
                                subprocess.Popen(val_cmd, stdout=f_log, stderr=subprocess.STDOUT)
                            print(f"✅ 后台测试已发出，训练不中断继续...\n")
...
if __name__ == "__main__":
    ...
    # 🔥 [新增] 断点续训与后台可视化参数
    if '--resume_from_checkpoint' not in existing_args:
        parser.add_argument("--resume_from_checkpoint", type=str, default=None, 
                          help="断点 safetensors 文件路径")
    if '--run_validation' not in existing_args:
        parser.add_argument("--run_validation", action="store_true", 
                          help="保存权重时，在后台自动启动推理生成视频")
    if '--val_script' not in existing_args:
        parser.add_argument("--val_script", type=str, default="examples/wanvideo/model_training/validate_full/Wan2.2-TI2V-5B.py", 
                          help="推理脚本路径")
