...
class WanTrainingModule(DiffusionTrainingModule):
    def __init__(
        ...
        if use_structure_encoder:
            ...
            # 集成到pipeline
            integrate_structure_encoder_to_pipeline(
                self.pipe, 
                self.structure_encoder, 
                freeze_dit=False  # 暂时不在这里冻结
            )
            
            # 🔥🔥🔥 新增：冻结所有Wan模型，只训练结构编码器
            if not freeze_structure_encoder:  # freeze_structure_encoder=False时训练编码器
                print("[INFO] ========================================")
                print("[INFO] TRAINING MODE: Structure Encoder Only")
                print("[INFO] ========================================")
...
def launch_training_task_torchrun(
    ...
    for epoch_id in range(args.num_epochs):
        ...
        if is_main_process:
            ...
            print(f"\nEpoch {epoch_id+1} completed. Average loss: {avg_loss:.4f}")
            
            if not args.save_steps:
                save_checkpoint(model, model_logger, epoch_id, args, epoch=True)
